...
declare
  student_id uuid;
  v_order_id constant uuid := 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa';
  product_id constant uuid := 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb';
  first_result jsonb;
...
begin
  ...
  if student_id is null then raise exception 'VERIFY_STUDENT_REQUIRED'; end if;

  insert into public.key_products (
    id, code, name, product_kind, attempt_count, price_amount, currency,
    valid_days, is_active
  ) values (
    product_id, 'VERIFY_THUEAPI', 'Verify ThueAPIBank', 'bundle', 3, 125000,
    'VND', 30, true
  );

  insert into public.purchase_orders (
    id, student_id, product_id, status, amount, currency, product_snapshot,
    idempotency_key, payment_code, expires_at, created_at
  ) values (
    v_order_id, student_id, product_id, 'pending', 125000, 'VND',
    jsonb_build_object('attempt_count', 3, 'valid_days', 30),
    'verify-thueapi-idempotency', 'THPTABC123456789', now() + interval '1 day',
    now() - interval '1 minute'
  );

  first_result := public.process_bank_payment(
    'thueapibank', 'tx-verify-1', now(), 'in', 125000,
    'THPTABC123456789', 'THPTABC123456789', '1017588888', '1017588888',
    'VCB', 'VCB', 'FT-VERIFY-1', '{"fixture":"first"}'::jsonb,
    repeat('a', 64)
  );
  duplicate_result := public.process_bank_payment(
    'thueapibank', 'tx-verify-1', now(), 'in', 125000,
    'THPTABC123456789', 'THPTABC123456789', '1017588888', '1017588888',
    'VCB', 'VCB', 'FT-VERIFY-1', '{"fixture":"first"}'::jsonb,
    repeat('a', 64)
  ...
  if (first_result ->> 'accepted')::boolean is not true
     or (first_result ->> 'duplicate')::boolean is not false then
    raise exception 'FIRST_PAYMENT_NOT_ACCEPTED: %', first_result;
  end if;
  if (duplicate_result ->> 'duplicate')::boolean is not true then
    raise exception 'DUPLICATE_NOT_DETECTED: %', duplicate_result;
  end if;
  if (select status from public.purchase_orders where id = v_order_id) <> 'fulfilled' then
    raise exception 'ORDER_NOT_FULFILLED';
  end if;
  if (select count(*) from public.payment_events where payment_events.order_id = v_order_id) <> 1 then
    raise exception 'PAYMENT_EVENT_NOT_IDEMPOTENT';
  end if;
  if (select count(*) from public.exam_keys where source_order_id = v_order_id) <> 1 then
    raise exception 'KEY_NOT_IDEMPOTENT';
  end if;
  if (select assigned_to from public.exam_keys where source_order_id = v_order_id) is distinct from student_id then
    raise exception 'KEY_NOT_ASSIGNED_TO_BUYER';
  end if;

  first_lease := public.claim_payment_provider_lease(
    'thueapibank', '11111111-1111-4111-8111-111111111111', 20, false
  );
  second_lease := public.claim_payment_provider_lease(
    'thueapibank', '22222222-2222-4222-8222-222222222222', 20, false
  );
  if (first_lease ->> 'acquired')::boolean is not true
     or (second_lease ->> 'acquired')::boolean is not false then
    raise exception 'LEASE_EXCLUSIVITY_FAILED';
  end if;
  perform public.complete_payment_provider_poll(
    'thueapibank', '11111111-1111-4111-8111-111111111111',
    'cursor-verified', true, null, false
  );
end
$$;
rollback;
select 'PASS: fulfilled=1 event=1 key=1 duplicate=true lease_exclusive=true rollback=true' as verification_status;